...
public readonly struct ColumnWidth
{
    ...
    /// <summary>
    /// Defines the default font family for <see cref="ColumnWidth.CharacterWidth"/>
    ...
    public ColumnWidth(string literal, string fontFamily, double fontSize, FontAttributes attributes)
    {
        ...
        FontFamily = string.IsNullOrEmpty(fontFamily) ? DefaultFontFamily : fontFamily;
        ...
        // The width returned from Measure doesn't current match the width returned
        ...
        // For now, adjust the width until a better solution is found.
        // Using @3 for the width does not illustrate this problem because Character widths
        // use the letter 'W' for the repeating character which is generally sufficient
        // for most 3-character patterns.
        Value = TextUtilities.Measure(literal, fontFamily, attributes, fontSize).Width + 4;
    }
    ...
    {
        if (string.IsNullOrEmpty(FontFamily) || object.ReferenceEquals(FontFamily, DefaultFontFamily))
        {
            return;
        }
